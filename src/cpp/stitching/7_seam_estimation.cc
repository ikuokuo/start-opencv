...
int main(int argc, char const *argv[]) {
  ...
  auto parser = optparse::OptionParser()
      .usage("usage: %prog [options]\n"
             "       %prog --work_megapix 0.6 --features orb --matcher homography --match_conf 0.3 --range_width -1 --conf_thresh 1 --save_graph graph.txt --estimator homography --ba ray --ba_refine_mask xxxxx --wave_correct horiz --warp compressedPlaneA2B1 --seam_megapix 0.1 --expos_comp channels_blocks --expos_comp_nr_feeds 1 --expos_comp_nr_filtering 2 --expos_comp_block_size 32 --seam gc_colorgrad")
  ...
  logger->Reset("Exposure Compensation");
  {
    ...
    Ptr<detail::ExposureCompensator> compensator = detail::ExposureCompensator::createDefault(type);
